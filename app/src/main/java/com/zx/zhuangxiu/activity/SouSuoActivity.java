...
public class SouSuoActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    private void getSearchInfo() {
        if (title.equals("")) {
            ...
            Toast.makeText(SouSuoActivity.this, "请输入商品再搜索！", Toast.LENGTH_SHORT).show();
            return;
        } else {
            String url = URLS.sySearchShow(title);

            OkHttpUtils.get(url, new OkHttpUtils.ResultCallback<SearchShopBean>() {
                @Override
                public void onSuccess(SearchShopBean response) {
                    if (response.getResult() == 1) {
                        sySearchTwoList = response.getData();
                        ...
                        souSuoAdapter = new SouSuoAdapter(SouSuoActivity.this, sySearchTwoList);
                        mListview.setAdapter(souSuoAdapter);
                    } else {
                        ss_no.setVisibility(View.VISIBLE);
                        Toast.makeText(SouSuoActivity.this, "非常抱歉，没有您搜索的相关商品！", Toast.LENGTH_SHORT).show();
                    }
                }
    ...
    public boolean dispatchTouchEvent(MotionEvent ev) {
        if (ev.getAction() == MotionEvent.ACTION_DOWN) {
            View v = getCurrentFocus();        //当isShouldHideInput(v, ev)为true时，表示的是点击输入框区域，则需要显示键盘，同时显示光标，反之，需要隐藏键盘、光标
