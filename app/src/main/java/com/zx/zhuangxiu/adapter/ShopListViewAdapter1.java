...
public class ShopListViewAdapter1 extends BaseAdapter {

    private boolean isShow = true;//是否显示编辑/完成
    private List<ChakangouwucheBean.DataBean> shoppingCartBeanList;
    private CheckInterface checkInterface;
    private ModifyCountInterface modifyCountInterface;
    private Context context;


    public ShopListViewAdapter1(Context context) {
        this.context = context;
    }

    public void setShoppingCartBeanList(List<ChakangouwucheBean.DataBean> shoppingCartBeanList) {
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null) {
            convertView = LayoutInflater.from(context).inflate(R.layout.fragment_shop_item1, parent, false);
            holder = new ViewHolder(convertView);
            convertView.setTag(holder);
        } else {
            holder = (ViewHolder) convertView.getTag();
        }
        final ChakangouwucheBean.DataBean shoppingCartBean = shoppingCartBeanList.get(position);
        boolean choosed = shoppingCartBean.getChoosed();
        if (choosed){
            holder.ckOneChose.setChecked(true);
        }else{
            holder.ckOneChose.setChecked(false);
        }
//        String attribute = shoppingCartBean.getRecordName();
...
//            holder.tvCommodityAttr.setText(attribute+"");
//        }
        holder.tvCommodityName.setText(shoppingCartBean.getCdname());
        ...
        String imagUrl = shoppingCartBean.getPicture();
        Picasso.with(context).load(URLS.HTTP+imagUrl).error(R.mipmap.logo_zhanwei).fit().into(holder.ivShowPic);
    ...
    class ViewHolder {
        ...
        public ViewHolder(View itemView) {
            ...
            ivShowPic = (RoundImageView) itemView.findViewById(R.id.iv_show_pic);
