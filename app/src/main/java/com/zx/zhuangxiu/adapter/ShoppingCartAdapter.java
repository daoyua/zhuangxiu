...
//        final ShoppingCartBean shoppingCartBean = shoppingCartBeanList.get(position);
      ...
      final boolean choosed = shoppingCartBean.getChoosed();
        if (choosed){
            holder.ckOneChose.setChecked(true);
        }else{
            holder.ckOneChose.setChecked(false);
        }
        String attribute = shoppingCartBean.getSimple();
        ...
        Picasso.with(context).load(URLS.HTTP+shoppingCartBean.getPicture()).error(R.mipmap.logo_zhanwei).fit().into(holder.ivShowPic);
...
//                        shoppingCartBean.setChoosed(((CheckBox) v).isChecked());
    ...
    class ViewHolder {
        ...
        TextView tvCommodityName, tvCommodityAttr, tvCommodityPrice, tvCommodityShowNum,ivSub, ivAdd;
        CheckBox ckOneChose;
        LinearLayout rlEdit;
